...
<script type="text/html" data-template-name="loop">
    ...
    <div class="form-row">
      <label for="node-input-varName"><i class="fa fa-tag"></i> Counter variable name </label>
      <input type="text" id="node-input-varName">
    </div>
    <div class="form-row">
        <label for="node-input-loopOver"><i class="fa fa-tasks"></i> Loop over:</label>
        <select id="node-input-loopOver">
            <option value="numbers">Numbers</option>
            <option value="list">List</option>
        </select>
        <!-- <input type="text" id="node-input-loopOver"> -->
...
<script type="text/javascript">
  RED.nodes.registerType('loop', {
    ...
    paletteLabel: 'loop',
    outputLabels: ["In-Loop", "After-Loop"],
    oneditprepare: function () {
      var node = this;

      $("#node-input-loopOver").on("change", function () {
        if (this.value === "list") {
          $("#list-details").show();
